...
const QString StrongStoreUuid = QLatin1String("60000000-0000-0000-0000-000000000001");
const QString ManagedRootUuid = QLatin1String("70000000-0000-0000-0000-000000000001");
...
private Q_SLOTS:
    ...
    void testStrongCompatibilityLockedRestartAndRelockRestore();
    void testStrongProtectInterruptedPublicationResumesWithVault();
...
void PrivacyStillItemTransactionTest::testStrongProtectUnprotectRoundTrip()
{
    ...
    managedRoot.markerUuid = QLatin1String("marker-strong");
    ...
    const PrivacyStillItemTransactionResult result =
        engine.protect(protect, password);
    QCOMPARE(result.status, PrivacyStillItemTransactionStatus::Protected);

    QCOMPARE(persistence.snapshot.containers.size(), 1);
    const PrivacyContainer container = persistence.snapshot.containers.first();
    QCOMPARE(container.kind, PrivacyContainerKind::StrongObject);
    QCOMPARE(container.storeUuid, StrongStoreUuid);
    QCOMPARE(container.objectRelativePath,
             QLatin1String("originals/") + ContainerUuid);

    QFile protectedObject(QDir(vaultDirectory.path()).filePath(
        QLatin1String("originals/") + ContainerUuid +
        QLatin1String("/0-synthetic.jpg")));
    QVERIFY(protectedObject.open(QIODevice::ReadOnly));
    QCOMPARE(protectedObject.readAll(), originalBytes);
    protectedObject.close();

    QFile publicProxy(sourcePath);
...
void PrivacyStillItemTransactionTest::
    testStrongCompatibilityLockedRestartAndRelockRestore()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    QTemporaryDir vaultDirectory;
    ...
    QCOMPARE(exposed.readAll(), originalBytes);
    exposed.close();
    const QString vaultObjectPath = QDir(vaultDirectory.path()).filePath(
        QLatin1String("originals/") + ContainerUuid +
        QLatin1String("/0-synthetic.jpg"));
    QVERIFY(QFileInfo::exists(vaultObjectPath));
    ...
    PrivacyStillItemTransactionEngine restarted(
        persistence, restartedRuntime, restartedCache);
    QCOMPARE(restarted.recover(publicRoot, unlock.transactionUuid).status,
             PrivacyStillItemTransactionStatus::AuthenticationRequired);

    const QString vaultObjectDirectory = QDir(vaultDirectory.path()).filePath(
        QLatin1String("originals/") + ContainerUuid);
    QVERIFY(QDir(vaultObjectDirectory).removeRecursively());

    QCOMPARE(restarted.recover(publicRoot, unlock.transactionUuid,
                               vaultDirectory.path(), StrongStoreUuid).status,
             PrivacyStillItemTransactionStatus::CompatibilityRelocked);

    QFile relocked(sourcePath);
    QVERIFY(relocked.open(QIODevice::ReadOnly));
    QVERIFY(relocked.readAll() != originalBytes);
    relocked.close();
    QFile restoredVault(vaultObjectPath);
    QVERIFY(restoredVault.open(QIODevice::ReadOnly));
    QCOMPARE(restoredVault.readAll(), originalBytes);
    restoredVault.close();
    QCOMPARE(persistence.snapshot.transactions.constFirst().state,
             PrivacyTransactionState::Complete);
}
...
void PrivacyStillItemTransactionTest::
    testStrongProtectInterruptedPublicationResumesWithVault()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    QTemporaryDir vaultDirectory;
    ...
    PrivacyStillItemTransactionEngine restarted(
        persistence, restartedRuntime, restartedCache);
    QCOMPARE(restarted.recover(publicRoot, ProtectUuid).status,
             PrivacyStillItemTransactionStatus::AuthenticationRequired);
    const PrivacyStillItemTransactionResult resumed =
        restarted.resumeAuthenticated(
            publicRoot, ProtectUuid, password, false,
            vaultDirectory.path(), StrongStoreUuid);
    QCOMPARE(resumed.status, PrivacyStillItemTransactionStatus::Protected);

    QFile proxied(sourcePath);
    QVERIFY(proxied.open(QIODevice::ReadOnly));
    QVERIFY(proxied.readAll() != originalBytes);
    proxied.close();
